...
-- INSERT INTO 테이블명 (지정컬럼1, 지정컬럼2, 지정컬럼3,.......) VALUES(값1,값2,값3,......);
-- 2. 전체 컬럼에 데이터를 추가하는 방법 -> 테이블에 있는 전체 컬럼의 값을 다 작성해야함.
...
VALUES('A1','L3');-- 주의! 지정된지 않은 컬럼에 NOT NULL제약조건이 설정되어 있으면 안된다!
SELECT * FROM DEPARTMENT;

--전체 컬럼(데이터)에 값 넣기
INSERT INTO DEPARTMENT VALUES('A2','오라클개발부','L4');
SELECT * FROM DEPARTMENT;
INSERT INTO DEPARTMENT VALUES('A3');-- NOT ENOUGH VALUES 에러발생
INSERT INTO DEPARTMENT VALUES('A3','JDBC개발부','L3',19);-- TO MANY VALUES 에러발생
INSERT INTO DEPARTMENT VALUES('A4',NULL,'L9');
SELECT * FROM DEPARTMENT;

--EMPLOYEE 테이블에 자신의 정보를 입력하기
-- 1. 전체컬럼에 대해 입력
-- 2. 필수컬럼에 대해 입력(NOT NULL 제약조건이 있는 컬럼에만 작성)
DESC EMPLOYEE;
INSERT INTO EMPLOYEE(EMP_ID, EMP_NAME,EMP_NO,JOB_CODE,SAL_LEVEL)
VALUES('999','유병승','010224-3456789','J3','S4');
SELECT *  FROM EMPLOYEE;

-- INSERT문에 서브쿼리를 이용해 보자
CREATE TABLE INSERT_TEST
AS SELECT EMP_ID, EMP_NAME, DEPT_TITLE
    FROM EMPLOYEE JOIN DEPARTMENT ON DEPT_CODE=DEPT_ID
    WHERE 1=2;
SELECT * FROM INSERT_TEST;
INSERT INTO INSERT_TEST VALUES('1','유지훈','개발팀');
...
--INSER ALL : 두개이상의 테이블에 한개SELECT문의 결과를 각각 값을 넣을때 사용
...
INSERT ALL
INTO EMP_HIRE_DATE VALUES(EMP_NAME, EMP_ID, HIRE_DATE)
...
--UPDATE문 : 한개 또는 그이상의 ROW의 값을 수정하는 명령어
--UPDATE 테이블명 SET 수정할컬럼명=수정할값, 수정할컬럼=수정할값.... [WHERE 조건식] *WHERE은 생략하지마! 큰일나!
...
--만약에 UPDATE문에서 WHERE절을 쓰지 않는다면??? -> 반드시 WHERE을 사용해야함.
...
UPDATE EMP_COPY 
--            SET SALARY=(SELECT SALARY FROM EMP_COPY  WHERE EMP_NAME='전지연') ,
...
WHERE EMP_NAME='임시환';
SELECT * FROM EMP_COPY WHERE EMP_NAME='임시환';
ROLLBACK;


-- DELETE : 조건에 맞는 ROW를 삭제하는 명령어
-- DELETE FROM 테이블 [WHERE 조건식] * WHERE을 생략하면 전체를 지우는 것
SELECT * FROM EMP_COPY;
DELETE FROM EMP_COPY WHERE EMP_NAME='유병승';

-- BONUS를 받지 않는 사원을 삭제
DELETE FROM EMP_COPY WHERE BONUS IS NOT NULL;

--DELETE사용시 주의할 사항 : 자신을 참조하고 있는 자식테이블이 있는 경우 삭제 불가능(FOREIGN KEY)


-- 전체 ROW를 삭제하는것 : TRUNCATE -> 1. DELETE보다 빠름... 2. ROLLBACK할 수 없음
...
INSERT INTO TBL_MERGE01 VALUES('USER01','유병승');
INSERT INTO TBL_MERGE01 VALUES('USER02','유지훈');
INSERT INTO TBL_MERGE02 VALUES('USER03','김가현');
INSERT INTO TBL_MERGE02 VALUES('USER02','박원재');

--MERGE INTO 기준이되는 테이블명
--USING 합칠테이블명 ON (동일여부를 판단하는 기준컬럼)
--WHEN MATCHED THEN DML구문
--WHEN NOT MATCHED THEN DML구문
MERGE INTO TBL_MERGE01 T1
USING TBL_MERGE02 T2 ON (TBL_MERGE01.ID=TBL_MERGE02.ID)
WHEN MATCHED THEN
...
-- ALTER : 테이블 등 오라클에서 제공하는 OBJECT를 수정할때 사용
-- 컬럼추가, 컬럼삭제, 컬럼명변경, 자료형변경(길이변경), DEFAULT값 설정
-- 제약조건 추가, 삭제, 이름을 변경
...
-- 컬럼를 추가할때 제약조건도 추가할 수 있을까?
ALTER TABLE TBL_USER_ALTER ADD(GENDER VARCHAR(10) CHECK(GENDER IN('남','여')));
SELECT * FROM TBL_USER_ALTER;
INSERT INTO TBL_USER_ALTER VALUES(3,'USER2','USER2','유저이','상봉','여');

-- 데이터가 있는 테이블에 컬럼을 추가할때 주의할점. NOT NULL제약조건 설정시 주의!
--ALTER TABLE TBL_USER_ALTER ADD(EMAIL VARCHAR2(50) NOT NULL DEFAULT 'NONE');
ALTER TABLE TBL_USER_ALTER ADD(EMAIL VARCHAR2(50) DEFAULT 'NONE' NOT NULL);
SELECT * FROM TBL_USER_ALTER;
INSERT INTO TBL_USER_ALTER(USER_NO,USER_ID,USER_PWD,USER_NAME,ADDRESS,GENDER) 
            VALUES(4,'USER3','USER3','유저삼','은평구','남');


-- 제약조건 추가하기
-- ALTER TABLE 테이블명 ADD 제약조건(컬럼) -> SYS00123제약조건명이 권장하지않음!
-- ALTER TABLE 테이블명 ADD CONSTRAINT 제약조건명 제약조건(컬럼명)
SELECT * FROM TBL_USER_ALTER;
ALTER TABLE TBL_USER_ALTER ADD CONSTRAINT TBL_USER_ID_UQ UNIQUE(USER_ID);
-- NOT NULL -> 안됩니다! 왜?? 추가가 아니라 수정!
...
--컬렁명 수정하기 
-- ALTER TABLE 테이블명 RENAME COLUMN 변경할 컬럼명 TO 변경명
ALTER TABLE TBL_USER_ALTER RENAME COLUMN USER_ID TO ID;
SELECT * FROM TBL_USER_ALTER;
--제약조건명 수정하기
ALTER TABLE TBL_USER_ALTER RENAME CONSTRAINT SYS_C007101 TO USER_NO_PK;

-- 테이블 삭제하기 : DROP TABLE 테이블명
DROP TABLE TBL_USER_ALTER;
SELECT * FROM TBL_USER_ALTER;
-- 테이블 삭제시 유의할 사항 -> 참조관계에 설정이 있다면 함부로 지울 수 없다.
CREATE TABLE REF_TEST(
    EMP_ID VARCHAR2(20) REFERENCES MEMBER(USER_ID),
    EMP_NAME VARCHAR2(20)
);
DROP TABLE MEMBER;--삭제 실패
--옵션설정해서 지울수가 있음.
-- CASCADE CONTSTRAINT : 제약조건을 삭제하고 테이블 지워라
DROP TABLE MEMBER CASCADE CONSTRAINT;

--DCL : DATA에 대한 접근권한 설정, 권한을 회수하는 기능/트렌젝션처리
-- 권한부여/회수 : GRANT, REVOKE
-- GRANT 권한명||ROLE명,권한명, 권한명..... TO 사용자명
-- 세부적인 내용(특정사용자의 특정테이블)에 대해 권한부여할때
-- GRANT 권한명 ON 테이블 TO 사용자명 / 다른사용자 소유의 테이블도 접근이 가능하게 만들 수 있음.
...
--사용자에게 부여된 권한을 확인하기
SELECT * FROM DBA_SYS_PRIVS--개별 권한 조회
WHERE GRANTEE='QWER';
--부여된 ROLE
...
--QWERT계정에 권한을 부여해서 KH소유 테이블에 접근하게 해보자.
SELECT * FROM EMPLOYEE;
...
VALUES('777','유지훈','770707-1777777','J5','S5');
...
-- 트렌젝션 : 작업단위
-- 트렌젝션은 INSERT, UPDATE, DELETE에 해당. -> 테이블의 ROW를 수정했을때
SELECT * FROM KH.TBL_USER;
GRANT SELECT ON KH.TBL_USER TO QWER;
INSERT INTO TBL_USER VALUES(2,'CO','123','유지훈','JI@JI.COM');
INSERT INTO TBL_USER VALUES(3,'CO','123','유지훈','JI@JI.COM');
INSERT INTO TBL_USER VALUES(4,'CO','123','유지훈','JI@JI.COM');
ROLLBACK;
COMMIT;

-- VIEW : 가상의테이블 SELECT문을 이용해서 실제테이블에서 데이터를 가져와 사용
-- VIEW도 하나의 ORALCE OBJECE이기 때문에  DDL로 생성, 수정, 삭제.
...
AS SELECT DEPT_CODE, FLOOR(AVG(SALARY)) AS 평균
    ...
    GROUP BY JOB_CODE
    UNION
    SELECT '총평균', FLOOR(AVG(SALARY)) AS평균
    FROM EMPLOYEE;
...
--VIEW의 특징
--1. 가상컬럼을 VIEW로 이용할 수 있다. 연산에 의해 계산된 컬럼, 단 무조건 별칭을 부여해야한다.
CREATE VIEW V_EMPSAL
AS SELECT EMP_ID, EMP_NAME, (SALARY+SALARY*NVL(BONUS,0))*12 AS SAL
    FROM EMPLOYEE;    
--VIEW테이블도 권한설정해서 이용할 수 있게 할 수 있다
...
-- VIEW의 데이터를 수정, 삽입, 삭제할 수 있을까? DML가능한가?
-- 제약조건이 있음. 가상컬럼은 아예 불가능함.
UPDATE V_EMPSAL SET SAL=5000000 WHERE EMP_NAME='방명수';

UPDATE V_EMPSAL SET EMP_NAME='김가현' WHERE EMP_NAME='전지연';
...
-- 2. 뷰에 포함되지 않은 베이스테이블의 컬럼 중에서 NOT NULL제약조건이 설정된 경우
CREATE VIEW V_EMP
AS SELECT EMP_ID, EMP_NO, EMP_NAME, EMAIL, PHONE, JOB_CODE, SAL_LEVEL
    FROM EMPLOYEE;

SELECT * FROM V_EMP;
INSERT INTO V_EMP VALUES('666','660616-126666','육육','66@66.COM','010123456','J2','S4');

SELECT * FROM EMPLOYEE;

--3 산술연산으로 구성된컬럼, 그룹함수, GROUP BY를 사용한 VIEW

-- DISTINCT 포함된 VIEW
-- 여러 테이블이 조인되어있는 VIEW * 예외적으로 삽입되는 경우도 있음.

--VIEW옵션 
-- OR REPLACE  : 이미 그 이름의  VIEW가 있으면 덮어쓰기시키는 명령어
SELECT * FROM USER_VIEWS;
CREATE OR REPLACE VIEW V_EMP
AS SELECT * FROM EMPLOYEE JOIN DEPARTMENT ON DEPT_CODE=DEPT_ID;


-- FORCE/NOFORCE : 실제테이블이 없어도 VIEW를 생성할 수 있게 해주는 옵션
...
-- WITH CHECK OPTION : SELECT문에 WHERE에 사용한 컬럼은 수정못하게 하는 옵션
...
UPDATE V_EMP_SAL SET SALARY=100 WHERE EMP_NAME='대북혼';
ROLLBACK;


-- WITH READ ONLY :  VIEW테이블을 수정하지 못하게 하는 옵션
CREATE OR REPLACE VIEW V_READ
AS SELECT EMP_NAME, DEPT_CODE, JOB_CODE, SALARY
FROM EMPLOYEE WHERE DEPT_CODE='D5' WITH READ ONLY;

UPDATE V_READ SET SALARY=10000;

--SEQUENCE
-- 자동번호 부여기
-- CREATE SEQUENCE 시퀀스명[옵션]
-- 시퀀스작동 예약어 -> 시퀀스.NEXTVAL, 시퀀스.CURRVAL
--기본 시퀀스 생성하기
...
DELETE FROM BOARD_SEQ WHERE BOARD_NO=2;
...
-- 동일한 세션에서  CURRVAL를 출력하려면 NEXTVAL을 출력하고  실행해야함.
SELECT SEQ_TEST.NEXTVAL FROM DUAL;

--SEQUENCE를 사용할 수 없는 구문
-- VIEW구문에서 사용불가
...
-- 프로시저, 함수 -> 이름이 있어서 재호출 가능한 PL/SQL구문
...
-- PL/SQL구문에서 SELECT문을 사용하는것은 조회한 결과를 변수에 저장하여 활용함
-- PL/SQL구문에서 SELECT문을 사용했을때는 반드시 INTO를 사용해야함.(변수에 대입하는 것)
DECLARE
    V_EMP_ID EMPLOYEE.EMP_ID%TYPE;
BEGIN
    SELECT EMP_ID
...
BEGIN
    FOR K IN 1..10 LOOP
        INSERT INTO INSERTEST VALUES('이거 PL/SQL문으로 입력하기'||K);
        COMMIT;
    END LOOP;
END;
/
...
-- 사번을 입력받아 이름을 조회하고 그 이름이 선동일이면 난 대표다! 출력하기
DECLARE 
...
-- 직책코드가 J1이면 대표다! J2면 임원이다 그외면 사원이다. 출력시키기.
DECLARE
...
BEGIN
    ...
    IF V_JOBCODE='J1' THEN DBMS_OUTPUT.PUT_LINE('대표다');
    ELSIF V_JOBCODE='J2' THEN DBMS_OUTPUT.PUT_LINE('임원이다');
...
BEGIN
    INPUTVALUE := '&수입력';
    CASE INPUTVALUE
        WHEN 1 THEN DBMS_OUTPUT.PUT_LINE(INPUTVALUE||'를 입력함 1은 첫번째');
        WHEN 2 THEN DBMS_OUTPUT.PUT_LINE(INPUTVALUE||'를 입력함 2은 두번째');
        WHEN 3 THEN DBMS_OUTPUT.PUT_LINE(INPUTVALUE||'를 입력함 3은 세번째');
        ELSE DBMS_OUTPUT.PUT_LINE(INPUTVALUE||'는 없어요');
    END CASE;
END;
/

-- CASE문으로 월급이 400이상이면 고액 200이상이면 연봉, 그이하면.....슬프다.... 출력하기
-- CASE문을 잘 생각하면 답 나옴.
...
BEGIN
    ...
    CASE 
        WHEN V_SALARY >= 4000000 THEN DBMS_OUTPUT.PUT_LINE('고액연봉자');
        WHEN V_SALARY >= 2000000 THEN DBMS_OUTPUT.PUT_LINE('연봉자');
        ELSE DBMS_OUTPUT.PUT_LINE('슬프다');
    END CASE;
END;
/
SELECT * FROM EMPLOYEE;
...
-- EMPLOYEE에 있는 10의 ROW출력하기
-- EMP_ID, EMP_NAME, SALARY 시작은 EMP_ID 200부터 209까지출력
...
--매개변수 있는 프로시져 생성하기
-- IN : 프로시져실행시 필요한 값을 받는 변수(일반 매개변수 개념)
...
--FUNCTION 활용하기
--FUNCTION은 PROCEDURE와 유사 / RETURN값을 갖는다.
...
-- EMP_ID를 전달받아서 연봉을 계산해저 출력해주는 함수
...
END;
/
SELECT CALC_SALARY(201) FROM DUAL;
