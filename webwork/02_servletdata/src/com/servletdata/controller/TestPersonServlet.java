...
public class TestPersonServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		double height=Double.parseDouble(request.getParameter("height")==null?"0.0":request.getParameter("height"));
		
		String color=request.getParameter("color");
	
		//여러개의 값의 parameter를 가져올때
...
//		}
		...
		//응답페이지 작성하기
		//클라이언트 응답하기 위해서는 HttpServletReponse객체를 이용한다.
		//response객체는 응답하기위한 각종 정보를 가지고 있는 객체이다.
		
		
		//1. 응답하는 방식을 결정 -> response.setContentType();
		//전송되는 데이터의 MIME타입과 인코딩방식을 결정
		response.setContentType("text/html;charset=utf-8");
		//2.클라이언트에게 데이터를(페이지내용을) 전송하기 위해 Stream을 가져온다.
		// response.getWriter() : 문서(문자자체) / response.getOutputStream() : 바이너리파일(동영상, 사진, 일반파일, 노래)
		...
		html+="</ul>";
		html+="<h3>좋아하는 동물은 "+animal+"이다.</h3>";
		html+="<h4>이용해주셔서감사합니다.! ^.~ </h4>";
