...
public class ShareDataServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		//was에서 사용할 수 있는 데이터저장 객체를 알아보자
		//1. ServletContext : 서버실행시 부터 서버 종료시까지
		//2. HttpSession : session생성시 부터 session 종료시까지
		//3. HttpServletRequest : 요청된 순간부터 요청이 종료(응답할)될때까지
		//ServletContext객체 생성
		...
		//String str=obj; -> downcasting 강제적 형변황을 해야함.
		
		//저장한 데이터 출력하기
		//저장되 데이터를 출력할때는 저장된 key값을 기준으로 출력함.
		//getAttribute("key") -> 
